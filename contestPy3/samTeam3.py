...
class ReflexCaptureAgent(CaptureAgent):
  ...
  def bfsPulse(self, gameState):
    pos = Cell(p1, 0, self.getMazeDistance(p1,p2), None)
    openqueue = [pos]
    closedqueue = []
    while pos != p2:
      closedqueue.append(pos)
      openqueue.remove(pos)
      actions = gameState.getLegalActions(pos.value)
      for a in actions:
        successor = self.getSuccessor(gameState, a)
        dfe = self.getMazeDistance(successor,p2)
        openqueue.append(Cell(successor, pos.dfs + 1, dfe, pos))
